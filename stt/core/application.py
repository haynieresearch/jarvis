...
class core(object):
    def __init__(self, use_mic=USE_STANDARD_MIC, batch_file=None):
        ...
        if not os.access(paths.CONFIG_PATH, os.W_OK):
            self._logger.critical("Config dir %s is not writable. STT " +
                                  "won't work correctly.",
                                  paths.CONFIG_PATH)

        # FIXME: For backwards compatibility, move old config file to newly
        #        created config dir
        old_configfile = os.path.join(paths.PKG_PATH, 'config.yaml')
        new_configfile = paths.config('config.yaml')
        ...
        try:
            ...
            audio_engine_slug = 'defaultaudio'
        ...
        try:
            ...
            keyword = 'Pi'
        self._logger.info("Using wake word '%s'", keyword)
